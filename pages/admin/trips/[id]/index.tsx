...
export default function EditProgram() {
    ...
    (async () => {
      try {
        ...
        setFormData({
          ...
          priceExclude:      data.priceExclude ?? "",
  ...
  const setPriceExclude      = (v: string) => setFormData(p => ({ ...p, priceExclude: v }));
  const setPaymentConditions = (v: string) => setFormData(p => ({ ...p, paymentConditions: v }));
  const setCancellationTerms = (v: string) => setFormData(p => ({ ...p, cancellationTerms: v }));
  const setGeneralConditions = (v: string) => setFormData(p => ({ ...p, generalConditions: v }));

  const setHotelField = (index: number, field: keyof HotelItem, value: string) => {
    setFormData((prev) => {
      const hotels = [...prev.hotels];
      hotels[index] = {
        ...(hotels[index] || { name: '', website: '', image: '' }),
        [field]: value,
      };
      return { ...prev, hotels };
    });
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-xl shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-10">
                ...
                {formData.hotels.map((hotel, index) => (
                  <div key={index} className="grid md:grid-cols-3 gap-3 p-3 rounded-lg border border-gray-200 bg-white">
                    <input
                      type="text"
                      value={hotel.name}
                      onChange={(e) => setHotelField(index, 'name', e.target.value)}
                    ...
                    <input
                      type="url"
                      value={hotel.website}
                      onChange={(e) => setHotelField(index, 'website', e.target.value)}
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        type="url"
                        value={hotel.image}
                        onChange={(e) => setHotelField(index, 'image', e.target.value)}
          ...
          {/* -------------------------------------------------- */}
          {/*  Price options & conditions                       */}
          {/* -------------------------------------------------- */}
          <section className="space-y-6">
            <div>
              <label className="block mb-2 font-medium">Ce qui est inclus (Inclus)</label>
              <ReactQuill value={formData.priceInclude} onChange={setPriceInclude} />
            </div>

            <div>
              <label className="block mb-2 font-medium">Ce qui n&lsquo;est pas inclus (Non Inclus)</label>
              <ReactQuill value={formData.priceExclude} onChange={setPriceExclude} />
            </div>

            <div>
              <label className="block mb-2 font-medium">Conditions de paiement</label>
              <ReactQuill value={formData.paymentConditions} onChange={setPaymentConditions} />
            </div>

            <div>
              <label className="block mb-2 font-medium">Conditions d&lsquo;annulation et de remboursement</label>
              <ReactQuill value={formData.cancellationTerms} onChange={setCancellationTerms} />
            </div>

            <div>
              <label className="block mb-2 font-medium">Conditions générales</label>
          ...
          <section className="p-4 bg-gray-50 rounded-lg border">
            ...
            <p className="text-sm text-gray-500 mb-3">
              Créez une carte sur{" "}
              <a href="https://www.google.com/maps/d/" target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">
                Google My Maps
              </a>
              , puis collez le lien d&lsquo;intégration ici.
            </p>
            <input
              type="url"
              name="mapEmbedUrl"
